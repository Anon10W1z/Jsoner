...
public class JsonerPotionEffect {
	...
	private JsonerPotionEffect(PotionEffect potionEffect) {
		...
		this.curativeItems = potionEffect.getCurativeItems().stream().map(itemstack -> new JsonerItem(itemstack.getItem(), itemstack.getMetadata())).collect(Collectors.toList());
	}
